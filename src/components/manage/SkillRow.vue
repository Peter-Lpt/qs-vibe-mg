...
const { allAgentStatuses, groupedStatuses, syncedCount, totalCount, vibeSource } =
...
async function handleAction(status: InstanceType<typeof Object> extends never ? never : ReturnType<typeof useSkillAgentStatus>["allAgentStatuses"]["value"][number]) {
...
watch(isExpanded, (v) => {
  if (!v) selectedAgents.value.clear();
});
...
<template>
  ...
  >
    ...
    <div v-if="skill.has_conflict && isExpanded" class="px-3 pb-3">
      ...
      >
        ...
        <div class="space-y-1 pl-3.5">
          ...
          >
            ...
            >
              {{ resolvingConflict === item.agent.id ? "..." : actionLabel(t, item.action) }}
            </button>
            <span
              v-else
