...
UI 流程：设置页 → "添加 Agent" → 填写名称 + skills 路径 → 保存到 `.vab-config.json` → 主面板显示新 agent。
...
fn check_symlink_permission() -> SymlinkCapability {
    ...
    match result {
        ...
        Err(_) => {
            ...
            let test_junc = std::env::temp_dir().join("vab_test_junc");
...
|------|-------------|---------|---------|
| install | skill 文件夹的元数据（SKILL.md 内容 + 文件列表） | `.vab-history.json` 内嵌 | 单条 ≤ 1MB |
| delete | skill 文件夹完整备份 | `.vab-history/snapshots/{id}/` | 总量 ≤ 100MB |
...
逆操作映射：
  ...
  install     → 删除 skill 文件夹（从 ~/.vab-skills/ 移除）
  delete      → 从 snapshot 恢复文件夹到 ~/.vab-skills/
...
│ │ ~/.vab-skills/my-skill       │ │  │  my-skill    [symlink][×]││
│ │ 关联：claude✓ opencode✓      │ │  └──────────────────────────┘│
│ │ [symlink ▾]                  │ │                             │
│ └──────────────────────────────┘ │  ┌─ pi-agent ○ 未检测 ────┐ │
│                                  │  │  （未关联任何 skill）    │ │
...
│ │ ~/.vab-skills/another-sk     │ │  │  （未关联任何 skill）    │ │
│ │ 关联：无                      │ │  └──────────────────────────┘│
│ │ [symlink ▾]                  │ │                             │
│ └──────────────────────────────┘ │                             │
├────────────────────────────────┴─────────────────────────────┤
...
tracing_subscriber::fmt()
    ...
    .with_env_filter(
        tracing_subscriber::EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| "qs_vab_mg=debug".into())
    )
    .init();

// 日志文件位置
// Windows: %LOCALAPPDATA%/qs-vab-mg/logs/
// macOS: ~/Library/Logs/qs-vab-mg/
// Linux: ~/.local/share/qs-vab-mg/logs/
```
