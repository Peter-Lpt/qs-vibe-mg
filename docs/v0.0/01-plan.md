...
│   │   ├── commands/
│   │   │   ├── mod.rs
│   │   │   ├── skills.rs         # list_skills, get_skill_detail
│   │   │   ├── sync.rs           # create_link, remove_link, list_links
│   │   │   └── agents.rs         # list_agents, detect_agents
│   │   ├── models/
│   │   │   ├── mod.rs
│   │   │   ├── skill.rs
│   │   │   └── agent.rs
...
pub struct Skill {
    ...
    pub path: String,              // ~/.vab-skills/{id} 绝对路径
    pub linked_agents: Vec<String>, // 已关联的 agent id 列表（v0.0 简化）
}
```
...
### F01 Skill 扫描与展示
- [ ] 扫描 ~/.vab-skills/ 下所有子文件夹
- [ ] 每个子文件夹读取 SKILL.md，解析 frontmatter（name、description）
- [ ] 解析失败时用文件夹名作为 name，description 为空
- [ ] 返回 Skill 列表给前端

### F02 Agent 检测与展示
- [ ] 硬编码 7 个默认 agent 及其检测目录
- [ ] 检测目录存在 → detected = true
- [ ] 返回 Agent 列表给前端

### F03 Symlink 创建
- [ ] 前端选择 skill + agent → 调用后端创建 symlink
- [ ] 后端：~/.vab-skills/{skill} → {agent.skills_dir}/{skill}
- [ ] agent skills 目录不存在时自动创建
- [ ] 创建前检测是否已存在

### F04 Symlink 删除
- [ ] 前端点击已关联的 agent 标签 → 调用后端删除 symlink
- [ ] 后端：删除 {agent.skills_dir}/{skill}（仅删链接，不删源文件）

### F05 关联状态扫描
- [ ] 启动时扫描每个 agent 的 skills 目录
- [ ] 检测哪些是 symlink 且指向 ~/.vab-skills/
- [ ] 更新 Skill.linked_agents 和 Agent 关联信息
...
│                        │    my-skill [×]               │
│ ┌─ web-srch ────────┐ │                              │
│ │ 网页搜索工具       │ │  pi-agent ○ not detected     │
│ │ 关联: claude ✓     │ │                              │
│ │ [关联到...]        │ │  codex ○ not detected        │
│ └────────────────────┘ │                              │
│                        │                              │
│ ┌─ data-tool ───────┐ │                              │
│ │ 数据分析           │ │                              │
...
交互：
- Skill 卡片点击 [关联到...] → 下拉选择 agent → 创建 symlink
...
// v0.0 暴露的命令
#[tauri::command]
fn list_skills() -> Result<Vec<Skill>, VabError>

#[tauri::command]
fn list_agents() -> Result<Vec<Agent>, VabError>

#[tauri::command]
fn detect_agents() -> Result<Vec<Agent>, VabError>
...
> v0.1 追加 PermissionDenied、SymlinkFailed、ConfigCorrupted 等变体。
...
```json
{
  ...
  "agents": [
    ...
    { "id": "agents-shared", "name": "共享目录", "skills_dir": "~/.agents/skills", "enabled": true }
  ]
}
```
...
Step 1: 项目初始化
  ...
  - 配置 Tailwind CSS
  - 验证 pnpm tauri dev 跑通

Step 2: Rust 后端骨架
  - 定义 models（Skill、Agent）
  - 定义 errors（VabError）
...
Step 3: Vue 前端骨架
  ...
  - composables（useSkills、useAgents）
...
composables/                 + useHistory, useTheme
stores/                      + app.ts
locales/                     新增（i18n）
```
